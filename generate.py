...
try:
    ...
    from order_feed import generate_order_events
    from restaurant_feed import (
        init_restaurant_metadata,
        generate_kitchen_events,
        generate_capacity_signals,
        get_restaurant_zone,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--orders",                  type=int,   default=None,
                   help="Total orders to simulate (default: calculated from duration and rate)")
    p.add_argument("--orders-per-minute",       type=int,   default=20,
                   help="Average orders per minute (used if --orders is not set)")
    p.add_argument("--duration",                type=int,   dest="duration_minutes", default=None,
                   help="Duration in minutes (alias for --duration-minutes)")
    ...
    p.add_argument("--format",                  choices=["json", "avro", "both"], dest="output_format",
                   help="Alias for --output-format")
    ...
    p.add_argument("--verify",                  action="store_true",
                   help="Verify generated AVRO files after writing")
    return p.parse_args()
...
def verify_data(output_dir: str, schema_dir: str) -> None:
    """
    Reads generated AVRO files and asserts required fields exist.
    """
    if not AVRO_AVAILABLE:
        print("  [VERIFY] skipped — fastavro not installed.")
        return

    feeds = [
        ("order_lifecycle", ["event_time", "ingestion_time", "order_id", "restaurant_id", "courier_id", "zone_id"]),
        ("restaurant_events", ["event_time", "ingestion_time", "restaurant_id", "zone_id"])
    ]

    for feed_name, required_fields in feeds:
        avro_path = os.path.join(output_dir, f"{feed_name}_sample.avro")
        if not os.path.exists(avro_path):
            print(f"  [VERIFY] skipped {feed_name} (file not found)")
            continue

        print(f"  [VERIFY] {feed_name}...")
            ...
            for record in reader:
                ...
                for field in required_fields:
                    if field not in record:
                        print(f"    ❌ Field '{field}' missing in {feed_name}")
                        sys.exit(1)
...
def main() -> None:
    ...
    if args.orders is None:
        args.orders = args.duration_minutes * args.orders_per_minute
    ...
    print(f"    Orders:      {args.orders} (rate: {args.orders/args.duration_minutes:.1f}/min)")
    ...
    for rid in restaurants:
        try:
            ...
            from restaurant_feed import _restaurant_meta
    ...
    schema_dir = None
    for d in possible_schema_dirs:
        if os.path.isdir(d) and os.path.exists(os.path.join(d, "order_lifecycle.avsc")):
            schema_dir = d
            break
    
    if schema_dir is None:
        print("❌ Error: Could not find schemas/ directory with .avsc files.")
        sys.exit(1)
    ...
    if args.verify:
        verify_data(args.output_dir, schema_dir)
